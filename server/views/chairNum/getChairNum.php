<div style="display: flex; justify-content: space-between; align-items: center">
    <h5 class="card-header">Quản lý Số Ghế</h5>
...
<table class="table table-bordered table-hover" id="moviesData">
    ...
    <tbody class="t-body">
        ...
        <?php
            ...
            $sql = "SELECT * FROM so_ghe LIMIT $from, $max_results";
            $query = queryDB($sql);
...
<?php
    ...
    $result =  queryDB("SELECT COUNT(*) as Num FROM so_ghe");  
    ...
    if($page > 1){  
        $prev = ($page - 1);  
        echo "<a href=\"".$_SERVER['PHP_SELF']."?action=getChairNum&page=$prev\"><button class='trang'>Trang trước</button></a>&nbsp;";  
    }  
    ...
    for($i = 1; $i <= $total_pages; $i++){  
        if(($page) == $i){  
            ...
            echo "<a href=\"".$_SERVER['PHP_SELF']."?action=getChairNum&page=$i\"><button class='so'>$i</button></a>&nbsp;";  
        }  
    }  
    ...
    if($page < $total_pages){  
        $next = ($page + 1);  
        echo "<a href=\"".$_SERVER['PHP_SELF']."?action=getChairNum&page=$next\"><button class='trang'>Trang sau</button></a>";  
    }  
    echo "</center>";
?>