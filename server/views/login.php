<?php
  ...
  if(isset($_POST['login'])){
    ...
    $sql_login = "SELECT * FROM nhan_vien WHERE username = '$email' AND password = '$password'";
    $query = queryDB($sql_login);
    $field = mysqli_fetch_assoc($query);
    if($field){
      $_SESSION['user_id'] = $field['Ma_NV'];
      $_SESSION['username'] = $field['username'];
      header("Location: http://localhost/movie_booking_project_team2/server/views/");
      exit();
    }
    else {
